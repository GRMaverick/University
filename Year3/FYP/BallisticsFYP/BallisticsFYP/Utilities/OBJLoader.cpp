...
Geometry OBJLoader::Load(char* filename, ID3D11Device* _pd3dDevice, bool invertTexCoords)
{
	...
	if (!binaryInFile.good())
	{
		...
		else
		{
			...
			meshData.Topology = D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
...
std::vector<Geometry> OBJLoader::Load2(char* filename, ID3D11Device* _pd3dDevice, bool invertTexCoords)
{
	std::string binaryFilename = filename;
	binaryFilename.append("Binary");
	std::ifstream binaryInFile;
	...
	if (!binaryInFile.good())
	{
		...
		{
			std::vector<Geometry> geometries = std::vector<Geometry>();
			...
			while (!inFile.eof())
			{
				...
				else if (input.compare("o") == 0)
				{
					...
					std::cout << "New Object in File: " << input << std::endl;
				}
			}
