...
ID3D11Buffer* Runway::_vertexBuffer = nullptr;
ID3D11Buffer* Runway::_indexBuffer = nullptr;

UINT Runway::VBOffset = 0;
UINT Runway::VBStride = 0;
UINT Runway::IndexCount = 0;
...
HRESULT Runway::Initialise(ID3D11Device * pd3dDevice, ID3D11ShaderResourceView* texture, float width, float depth, int verticesCountX, int verticesCountZ)
{
	HRESULT hr;
...
HRESULT Runway::InitialiseVB(ID3D11Device * pd3dDevice, float width, float depth, UINT m, UINT n)
{
	HRESULT hr;
...
HRESULT Runway::InitialiseIB(ID3D11Device * pd3dDevice, float width, float depth, UINT m, UINT n)
{
	HRESULT hr;
...
HRESULT Runway::InitialiseData(ID3D11Device * pd3dDevice, float width, float depth, UINT m, UINT n)
{
	HRESULT hr;
...
void Runway::Draw(ID3D11Device * pd3dDevice, ID3D11DeviceContext * pImmediateContext)
{
	pImmediateContext->PSSetShaderResources(0, 1, &_pTexture);
	pImmediateContext->IASetVertexBuffers(0, 1, &Runway::_vertexBuffer, &Runway::VBStride, &Runway::VBOffset);
	pImmediateContext->IASetIndexBuffer(Runway::_indexBuffer, DXGI_FORMAT_R32_UINT, 0);
	pImmediateContext->DrawIndexed(Runway::IndexCount, 0, 0);
}