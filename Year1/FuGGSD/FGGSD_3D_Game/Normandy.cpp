...
Normandy::Normandy(OBJMesh* mesh){
...
void Normandy::Update(){
	//Updates ship
}
void Normandy::UpdateAngle(char key){
	_key = key;
	if (key == 'a')
		_aBool = true;
	if (key == 's')
...
// Setters
void Normandy::SetPosition(Vector3* position){
	_position = position;
}
void Normandy::SetTexture(Texture2D* texture){
	_texture = texture;
}
void Normandy::SetMesh(OBJMesh* mesh){
	_mesh = mesh;
}

// Getters
OBJMesh* Normandy::GetMesh(){
	return _mesh;
}
Vector3* Normandy::GetPostion(){
	return _position;
}
Texture2D* Normandy::GetTexture(){
	return _texture;
}
